...
3. SURF_QUEUE_AND_STATUS_MANAGEMENT - Queue and Status Management
   ...
   Actions: ACT_SEARCH_RECORDS (search_input_persistent), ACT_SELECT_RECORD (inline_edit), ACT_SAVE_RECORD (form_submit)
...
4. SURF_AGENT_WORKLOAD - Agent Workload
   ...
   Actions: ACT_SEARCH_RECORDS (search_input_persistent), ACT_SELECT_RECORD (inline_edit), ACT_FILTER_INSIGHTS (context_menu)
...
6. SURF_SETTINGS_AND_PREFERENCES - Settings and Preferences
   ...
   Actions: ACT_SAVE_RECORD (form_submit), ACT_RETRY_LOAD (secondary_button)
...
- Surface Bound: SURF_TICKET_OPERATIONS
- Trigger: User types in search or changes filters.
- Preconditions & Auth: Ticket data is loaded or recoverable.
...
- Preconditions & Auth: User can create Ticket records.
- Async Behavior: Open editor immediately; no network wait.
- Success Effect: SURF_TICKET_EDITOR opens with blank/default fields.
- Failure Effect: Show unavailable-state message if creation is blocked.
- Navigation After Success: target SURF_TICKET_EDITOR, method modal.
- Navigation After Failure: target same, preserve_form_data true.
...
- Surface Bound: SURF_TICKET_EDITOR
- Trigger: User submits the editor form.
- Preconditions & Auth: Required fields pass validation and user can save Ticket.
- Async Behavior: Disable submit, show loading, timeout after 10 seconds with retry.
- Success Effect: Persist changes and return to SURF_TICKET_OPERATIONS with the updated row/card visible.
- Failure Effect: Inline field errors for validation; banner/toast for system failure; draft is preserved.
- Navigation After Success: target SURF_TICKET_OPERATIONS, method replace.
- Navigation After Failure: target same, preserve_form_data true.
- State Changes: Ticket collection and active selection update.
...
- Success Effect: Replace error with current Ticket data or a helpful empty state.
...
- Purpose: Give the user the main operational view for inspecting, searching, filtering, and acting on Ticket data.
- Data Entities Bound: Ticket, ActivityEvent, Preference
- Core Content: summary metrics, primary list/board/table, filters, search, selected item preview, empty/loading/error states.
...
- Name: Ticket Editor
- Purpose: Let the user create, edit, validate, save, cancel, and recover Ticket changes.
- Data Entities Bound: Ticket, ValidationError
- Core Content: form fields, required/optional indicators, validation messages, save/cancel controls, unsaved-state feedback.
...
- Exit & Guard Rules: Save returns to SURF_TICKET_OPERATIONS with persisted changes; cancel preserves existing data and closes the editor.
- Auth Required: false
...
- Data Entities Bound: Ticket, ActivityEvent, Preference
- Core Content: queue/status lanes, SLA or priority markers, ownership, blockers, aging indicators, and next-action controls.
- Permitted Actions: ACT_SEARCH_RECORDS (control_hint: search_input_persistent), ACT_SELECT_RECORD (control_hint: inline_edit), ACT_SAVE_RECORD (control_hint: form_submit)
- Entry Points: SURF_TICKET_OPERATIONS
- Exit & Guard Rules: Status changes keep the user in the same Ticket context and expose recoverable failure feedback.
- Auth Required: false
...
- Name: Agent Workload
- Purpose: Show how Ticket work is assigned, overloaded, pending, or blocked across agents or owners.
- Data Entities Bound: Ticket, ActivityEvent
- Core Content: agent/owner list, workload counts, stale or overdue indicators, reassignment controls, and recent activity.
- Permitted Actions: ACT_SEARCH_RECORDS (control_hint: search_input_persistent), ACT_SELECT_RECORD (control_hint: inline_edit), ACT_FILTER_INSIGHTS (control_hint: context_menu)
- Entry Points: SURF_TICKET_OPERATIONS, SURF_QUEUE_AND_STATUS_MANAGEMENT
- Exit & Guard Rules: Reassignment preserves the selected Ticket and updates visible queue/status counts.
- Auth Required: false
...
- Entry Points: SURF_TICKET_OPERATIONS
- Exit & Guard Rules: No external BI/admin area; returns to SURF_TICKET_OPERATIONS.
- Auth Required: false
...
- Core Content: saved filters, default views, notification or density preferences when relevant, reset controls, and visible save feedback.
- Permitted Actions: ACT_SAVE_RECORD (control_hint: form_submit), ACT_RETRY_LOAD (control_hint: secondary_button)
- Entry Points: SURF_TICKET_OPERATIONS
...
- Entry Points: SURF_TICKET_OPERATIONS, SURF_INSIGHTS
- Exit & Guard Rules: Recovery returns to the active Ticket workflow and preserves unrelated state.
- Auth Required: false
